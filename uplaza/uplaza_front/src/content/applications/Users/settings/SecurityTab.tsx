...
function SecurityTab() {
  ...
  const passwordChange = async ()=>{
    ...
    try {
      const res = await axios.patch(ApiUrl+"/users/"+user.id,{password},{headers:{"Authorization":`Bearer ${token}`}})
  ...
  return (
    <Grid container spacing={3}>
      ...
      <Grid item xs={12}>
        <Box pb={2}>
          <Typography variant="h3">امنیت</Typography>
          <Typography variant="subtitle2">
            تنظیمات امنیتی خود را در زیر تغییر دهید
          </Typography>
        </Box>
        <Card>
          <List >
            <ListItem sx={{ p: 3 ,direction:"rtl" ,textAlign:"right"}}>
              <ListItemText
                primaryTypographyProps={{ variant: 'h5', gutterBottom: true }}
                secondaryTypographyProps={{
                  variant: 'subtitle2',
                ...
                primary="رمز عبور"
                secondary="از این قسمت میتوانید رمز عبور خود را تغییر دهید"
              />
              {
                !openField ?
                    <Button size="large" variant="outlined" onClick={e=>setOpenField(true)}>
                      تغییر رمز عبور
                    </Button>
                    :
                    <Button size="large" variant="outlined" onClick={passwordChange} endIcon={<DoneTwoToneIcon/>}/>
              }
              {
                openField &&
                  <TextField
